...
with tf.name_scope('Fully_connected_layer1'):
    ...
    W_fc1 = tf.Variable(tf.truncated_normal(shape=[32*32*64,1024]))
...
with tf.name_scope('Fully_connected_layer2'):
    W_fc2 = tf.Variable(tf.truncated_normal(shape=[1024,3],stddev=0.1))
    b_fc2 = tf.Variable(tf.constant(0.1,shape=[3]))
    h_fc2 = tf.matmul(h_fc1_drop,W_fc2)+b_fc2
    h_fc21 = tf.cast(h_fc2,tf.float64)
#输入数据的大小为[128x128x3]
...
#        train_step.run(feed_dict={x:train,y_:train_label,keep_prob:0.5})
    ...
    plt.show()
    if i%10==0:
        test_s,test_label_s = batch_process(test,test_label,10,False)
        train_accuray = accuracy.eval(feed_dict={x:test_s,y_:test_label_s,keep_prob:1.0},session=sess)
        print('train accuracy %s' % train_accuray)